...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BSc (Biology, ComputerScience) -> "!!SCIENCE!!"
            | BPharm -> "Meh, it's OK."
        ...
        randomOpinion (BSc (Biology, ComputerScience)) |> should equal "!!SCIENCE!!"
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = 
            ...
            let g=Node("To",e,f)
            let h=Node("To",f,f)
            h// <-- you may want to spread this over multiple lines and/or let-bindings ...!
