    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec len ls total=
                match ls with
                |[]->total
                |hd::tl->len tl (total+1)
            len xs 0
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec maplist list acc=
                match list with 
                |[]->List.rev acc
                |a::b->maplist b ((a+1)::acc)
            maplist xs []// write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            ...
            maplist xs []// write a function which doubles each element
   ...
   *)
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filtering f list=
                match list with 
                |a::b -> match ( f a) with
                    |true-> a ::filtering f b
                    |false-> filtering f b
                |_ -> []
            match xs with 
            |[]->[]
   ...
   // ... you can make a function to do that, right? ^_^.
    ...
    let ``12 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            ...
            match xs with 
            |[]->[]
            |_->filtering (f) xs  // write a function which filters based on the specified criteria
        filter (fun x -> x > 19) [9; 5; 23; 66; 4] |> should equal [23; 66]
        filter (fun x -> String.length x = 4) ["moo"; "woof"; "yip"; "nyan"; "meow"]
        |> should equal ["woof"; "nyan"; "meow"]
        filter (fun (a,b) -> a*b >= 14) [9,3; 4,2; 4,5] |> should equal [9,3; 4,5]
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec foldf list acc=
                match list with
                    |a::b -> foldf b (acc+a)
                    |_->acc
            match initialState with
            |_->foldf xs initialState
        ...
        let fold initialState xs =
                let rec foldf list acc=
                    match list with
                        ...
                        |_->acc
                match initialState with
                |_->foldf xs initialState // write a function to multiply the elements of a list
        ...
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec foldf list acc=
                match list with
                    ...
                    |_->acc
            match initialState with
            |_->foldf xs initialState// write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec existing list=
                match list with
                |[]->false
                |a::b->match f a with 
                        |false->existing b
                        |true->true
            existing xs// Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec intlist len acc=
                match len < n with
                    |true->intlist (len+1) (f len::acc)
                    |false->List.rev acc
            intlist 0 []// Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec tryfinfing list acc=
                match list with
                |[]-> None
                |a::b ->match (p a) with
                        |true->Some a
                        |false->tryfinfing b acc
            tryfinfing xs None 
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec chooseFun list acc=
                match list with 
                |[]->List.rev acc
                |a::b-> match p a with
                        |None->chooseFun b acc
                        |Some a->chooseFun b (a::acc)
            chooseFun xs []// Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapiFun list acc len=
                match list with 
                |[]->List.rev acc
                |a::b ->mapiFun b (f len a::acc) (len+1)
            mapiFun xs [] 0// Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
